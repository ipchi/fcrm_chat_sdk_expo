...
__export(index_exports, {
  ...
  ChatApiException: () => ChatApiException,
  ChatApiService: () => ChatApiService,
  ChatException: () => ChatException,
  ChatSocketService: () => ChatSocketService,
  ChatStorageService: () => ChatStorageService,
  FcrmChatProvider: () => FcrmChatProvider,
  UploadCancelledException: () => UploadCancelledException,
  applyConfigDefaults: () => applyConfigDefaults,
  canEditMessage: () => canEditMessage,
  createEmptyPaginatedMessages: () => createEmptyPaginatedMessages,
  generateSignature: () => generateSignature,
  getApiUrl: () => getApiUrl,
  getMessageEditedAt: () => getMessageEditedAt,
  getMessageOriginalContent: () => getMessageOriginalContent,
  getMsHeaderColor: () => getMsHeaderColor,
  getMsNameColor: () => getMsNameColor,
  ...
  parseChatAppRemoteConfig: () => parseChatAppRemoteConfig,
  parseChatMessage: () => parseChatMessage,
  parseMessageType: () => parseMessageType,
  parsePaginatedMessages: () => parsePaginatedMessages,
  parseSocketMessage: () => parseSocketMessage,
  useChatConnection: () => useChatConnection,
  useChatMessages: () => useChatMessages,
  useChatTyping: () => useChatTyping,
  useFcrmChat: () => useFcrmChat,
  useFcrmChatContext: () => useFcrmChatContext
});
...
function generateSignature(appKey, appSecret) {
  return import_js_sha256.sha256.hmac(appSecret, appKey);
}
...
var CancelToken = class {
  constructor() {
    ...
    this._source = import_axios.default.CancelToken.source();
  }
  /**
...
var ChatApiService = class {
  ...
  async getConfig() {
    ...
    try {
      ...
      const statusCode = import_axios.default.isAxiosError(error) ? error.response?.status ?? 0 : 0;
...
var ChatStorageService = class {
  ...
  async saveBrowserKey(browserKey) {
    await import_async_storage.default.setItem(this.browserStorageKey, browserKey);
  }
  /**
   * Get browser key from storage
   */
  async getBrowserKey() {
    return await import_async_storage.default.getItem(this.browserStorageKey);
  }
  /**
   * Clear browser key from storage
   */
  async clearBrowserKey() {
    await import_async_storage.default.removeItem(this.browserStorageKey);
  }
  /**
   * Save user data to storage
   */
  async saveUserData(userData) {
    await import_async_storage.default.setItem(this.userDataStorageKey, JSON.stringify(userData));
  }
  /**
   * Get user data from storage
   */
  async getUserData() {
    const data = await import_async_storage.default.getItem(this.userDataStorageKey);
    if (data) {
      try {
   ...
   */
  async clearUserData() {
    await import_async_storage.default.removeItem(this.userDataStorageKey);
  }
  /**
...
var FcrmChatContext = (0, import_react.createContext)(void 0);
function FcrmChatProvider({
  config,
  children,
  autoInitialize = false
}) {
  const configWithDefaults = (0, import_react.useMemo)(
    () => applyConfigDefaults(config),
    [config]
  );
  const apiServiceRef = (0, import_react.useRef)(null);
  const socketServiceRef = (0, import_react.useRef)(null);
  const storageServiceRef = (0, import_react.useRef)(null);
  ...
  const updateBrowser = (0, import_react.useCallback)(
    async (userData) => {
      if (!state.isInitialized) {
        throw new ChatException("Chat not initialized. Call initialize() first.");
      }
  ...
  const updateUserData = (0, import_react.useCallback)(
    async (data) => {
      if (!state.isInitialized) {
        throw new ChatException("Chat not initialized. Call initialize() first.");
      }
  ...
  const updateName = (0, import_react.useCallback)(
    (name) => updateUserData({ name }),
    [updateUserData]
  );
  const updatePhone = (0, import_react.useCallback)(
    (phone) => updateUserData({ phone }),
    [updateUserData]
  );
  const updateEmail = (0, import_react.useCallback)(
  ...
  const sendImage = (0, import_react.useCallback)(
  ...
  const getMessages = (0, import_react.useCallback)(
  ...
  const sendTyping = (0, import_react.useCallback)(
    (isTyping) => {
      if (state.browserKey) {
  ...
  const onMessage = (0, import_react.useCallback)((callback) => {
    const socketService = socketServiceRef.current;
    if (!socketService) {
      return () => {
      };
  ...
  const onConnectionChange = (0, import_react.useCallback)(
    (callback) => {
      const socketService = socketServiceRef.current;
      if (!socketService) {
        return () => {
  ...
  const onTyping = (0, import_react.useCallback)((callback) => {
    const socketService = socketServiceRef.current;
    if (!socketService) {
      return () => {
      };
  ...
  const getUserData = (0, import_react.useCallback)(async () => {
    return await storageServiceRef.current?.getUserData() ?? null;
  }, []);
  const checkIsRegistered = (0, import_react.useCallback)(async () => {
    return await storageServiceRef.current?.isRegistered() ?? false;
  }, []);
  const createCancelToken = (0, import_react.useCallback)(() => {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(FcrmChatContext.Provider, { value: contextValue, children });
}
function useFcrmChatContext() {
  const context = (0, import_react.useContext)(FcrmChatContext);
  if (context === void 0) {
...
function useChatMessages(options = {}) {
  ...
  const [pagination, setPagination] = (0, import_react2.useState)({ hasMore: false, currentPage: 0, lastPage: 1 });
...
function useChatConnection() {
  ...
  const [isConnected, setIsConnected] = (0, import_react3.useState)(contextIsConnected);
...
function useChatTyping(options = {}) {
  ...
  const typingTimeoutRef = (0, import_react4.useRef)(null);
  const isCurrentlyTypingRef = (0, import_react4.useRef)(false);
  ...
  const setTyping = (0, import_react4.useCallback)(
    (text) => {
      if (typingTimeoutRef.current) {
        clearTimeout(typingTimeoutRef.current);
  ...
  const sendTyping = (0, import_react4.useCallback)(
    (isTyping) => {
      if (typingTimeoutRef.current) {
        clearTimeout(typingTimeoutRef.current);
        typingTimeoutRef.current = null;
